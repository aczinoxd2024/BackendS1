...
export class BitacoraService {
   ...
   */
  async registrar(idUsuario: string, accion: AccionBitacora, tabla: string, ip: string): Promise<void> {
  try {
    if (!idUsuario || !accion || !tabla || !ip) {
      console.error('❌ Registro bitácora inválido:', { idUsuario, accion, tabla, ip });
